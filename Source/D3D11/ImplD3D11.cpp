...
static HostCopyLayoutD3D11 GetHostCopyLayout(const TextureD3D11& texture, const TextureRegionDesc& region) {
...
static Result CreateHostCopyTexture(DeviceD3D11& device, const TextureD3D11& texture, uint32_t width, uint32_t height, uint32_t depth, Dim_t mipNum, MemoryLocation memoryLocation, TextureD3D11*& hostCopyTexture) {
    TextureDesc textureDesc = {};
    textureDesc.type = texture.GetDesc().type;
    textureDesc.format = texture.GetDesc().format;
    textureDesc.width = (Dim_t)width;
    textureDesc.height = (Dim_t)height;
    textureDesc.depth = (Dim_t)depth;
    textureDesc.mipNum = mipNum;
    textureDesc.layerNum = 1;
    textureDesc.sampleNum = 1;

    Result result = device.CreateImplementation<TextureD3D11>(hostCopyTexture, textureDesc);
    if (result == Result::SUCCESS)
...
static Result NRI_CALL CopyHostMemoryToTexture(Queue& queue, const CopyHostMemoryToTextureDesc* copyDescs, uint32_t copyDescNum) {
    DeviceD3D11& deviceD3D11 = ((QueueD3D11&)queue).GetDevice();
    MultiThreadProtection multiThreadProtection(deviceD3D11);
    ID3D11DeviceContextBest* context = deviceD3D11.GetImmediateContext();
    ...
    for (uint32_t i = 0; i < copyDescNum; i++) {
        ...
        if (IsWholeSubresource(copyDesc.dstRegion, layout)) {
            ...
            Result result = CreateHostCopyTexture(deviceD3D11, texture, textureDesc.width, textureDesc.height, textureDesc.depth, copyDesc.dstRegion.mipOffset + 1, MemoryLocation::HOST_READBACK, readbackTexture);
            if (result != Result::SUCCESS)
                return result;

            uint32_t readbackSubresource = copyDesc.dstRegion.mipOffset;
            ...
            Vector<uint8_t> subresourceData(deviceD3D11.GetStdAllocator());
...
static Result NRI_CALL CopyTextureToHostMemory(Queue& queue, const CopyTextureToHostMemoryDesc* copyDescs, uint32_t copyDescNum) {
    ...
    for (uint32_t i = 0; i < copyDescNum; i++) {
        ...
        const TextureDesc& textureDesc = srcTexture.GetDesc();
        uint32_t stagingWidth = useBox ? layout.width : textureDesc.width;
        uint32_t stagingHeight = useBox ? layout.height : textureDesc.height;
        uint32_t stagingDepth = useBox ? layout.depth : textureDesc.depth;
        Dim_t stagingMipNum = useBox ? 1 : copyDesc.srcRegion.mipOffset + 1;
        uint32_t stagingSubresource = useBox ? 0 : copyDesc.srcRegion.mipOffset;

        TextureD3D11* stagingTexture = nullptr;
        Result result = CreateHostCopyTexture(deviceD3D11, srcTexture, stagingWidth, stagingHeight, stagingDepth, stagingMipNum, MemoryLocation::HOST_READBACK, stagingTexture);
        if (result != Result::SUCCESS)
            return result;
